...
test("admin delete actions are guarded, audited and exposed in admin UI", () => {
  ...
  assert.match(deleteButton, /<Dialog>/)
  assert.match(deleteButton, /Confirmer la suppression/)
...
test("Clerk auth components are localized and styled for the French app", () => {
  const clerkTheme = readFileSync("src/components/auth/themed-clerk-provider.tsx", "utf8")
  const rootLayout = readFileSync("src/app/layout.tsx", "utf8")
  const loginPage = readFileSync("src/app/connexion/[[...sign-in]]/page.tsx", "utf8")

  assert.match(JSON.stringify(packageJson.dependencies), /@clerk\/localizations/)
  assert.match(rootLayout, /<ThemedClerkProvider>/)
  assert.match(clerkTheme, /import \{ frFR \} from "@clerk\/localizations"/)
  assert.match(clerkTheme, /localization=\{frFR\}/)
  assert.match(clerkTheme, /resolvedTheme === "dark" \? dark/)
  assert.match(clerkTheme, /formButtonPrimary/)
...
test("Prompt 4 checkout validates trusted server inputs", () => {
  ...
  assert.match(financeCheckout, /donorNameSnapshot: donorName/)
  assert.match(financeCheckout, /donorEmailSnapshot: donorEmail/)
  assert.match(financeCheckout, /Adresse email invalide/)
...
test("Prompt 4.5 finance totals and filters separate online and verified direct donations", () => {
  ...
  assert.match(readFileSync("src/components/admin/finance-overview.tsx", "utf8"), /Dons vérifiés/)
  assert.match(readFileSync("src/components/admin/finance-overview.tsx", "utf8"), /Total restant/)
